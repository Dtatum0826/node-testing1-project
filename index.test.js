...
describe('[Exercise 1] trimProperties', () => {
  test('[1] returns an object with the properties trimmed', () => {
   ...
   test('[2] returns a copy, leaving the original object intact', () => {
    const input = { foo: '  foo ', bar: 'bar ', baz: ' baz' };
    const original = { ...input }; // Make a copy of the input object
    utils.trimProperties(input);

    expect(input).toEqual(original); // Assert that the original object remains unchanged
  })
})
...
expect(result).toBe(true)
   ...
   test('[21] reejcts an error with the message "number must be a number" if passed a non-number type',async () => {
try{
...
expect(err.message).toMatch(/number must be a number/i)
   ...
   test('[22] rejects an error with the message "number must be a number type"if passed NaN',async () => {
    try{
